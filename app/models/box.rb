class Box < ActiveRecord::Base
  ...
  scope :first_grid_row, in_grid.where("grid_position <= 3 AND city_id = ? ",1)
  scope :second_grid_row, in_grid.where("grid_position > 3 AND city_id = ?",1)
  scope :in_carousel, where("carousel_position is not null AND city_id = ?", 1).order("carousel_position ASC")
